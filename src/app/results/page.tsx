...
import { ArrowLeft, Activity, AlertTriangle, CheckCircle2, XCircle, MapPin, Calendar, Users, Stethoscope, FileText, Brain, Shield, Hospital, Target, Navigation, Loader2, ChevronDown, Info } from "lucide-react";
...
function CriterionRow({ criterion }: { criterion: CriterionExplanation }) {
  ...
  return (
    <div className="border-b-2 border-black/10 p-3 flex items-center gap-3">
      ...
      <div className="flex-1">
        <span className="font-mono text-[10px] uppercase text-black/50">
          {criterion.category}{criterion.isML && " ML"}
        </span>
        <p className="font-mono text-xs font-bold">{criterion.criterion}</p>
      </div>
      <span className={`font-heading text-xs font-black uppercase ${
        criterion.status === "pass" ? "text-lime-green" : 
        criterion.status === "fail" ? "text-hot-coral" : "text-cyber-yellow"
      }`}>
        {criterion.status}
      </span>
    </div>
...
function ScoreBreakdown({ scores, confidence }: { scores: ScoreComponents; confidence: number }) {
  return (
    <div className="bg-black/5 p-4 border-t-2 border-black">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs font-mono">
        <div className="flex justify-between">
          <span>Rule ({Math.round(SCORING_WEIGHTS.rule * 100)}%)</span>
          <span className="font-bold">{scores.ruleScore}</span>
        </div>
        <div className="flex justify-between">
          <span>Embedding ({Math.round(SCORING_WEIGHTS.embedding * 100)}%)</span>
          <span className="font-bold">{scores.embeddingScore}</span>
        </div>
        <div className="flex justify-between">
          <span>LLM ({Math.round(SCORING_WEIGHTS.llm * 100)}%)</span>
          <span className="font-bold">{scores.llmScore}</span>
        </div>
        <div className="flex justify-between">
          <span>Geo ({Math.round(SCORING_WEIGHTS.geo * 100)}%)</span>
          <span className="font-bold">{scores.geoScore}</span>
        </div>
      </div>
      <div className="mt-2 pt-2 border-t border-black/20 flex justify-between font-heading font-black">
        <span>COMPOSITE</span>
        <span className="text-lime-green">{confidence}%</span>
      </div>
    </div>
  );
}
...
function TrialCard({ trial, isExpanded, onToggle }: { trial: TrialMatch; isExpanded: boolean; onToggle: () => void }) {
  ...
  const statusBadge = trial.status === "pass" 
    ? <span className="bg-lime-green px-2 py-1 text-xs font-heading font-black uppercase border-2 border-black">ELIGIBLE</span> 
    : <span className="bg-hot-coral px-2 py-1 text-xs font-heading font-black uppercase border-2 border-black">INELIGIBLE</span>;
  
  return (
    <div className="border-brutal shadow-brutal-sm bg-white">
      <button onClick={onToggle} className="w-full p-4 md:p-6 text-left">
        <div className="flex items-start justify-between">
          <div>
            <div className="font-mono text-xs text-black/50 mb-1">{trial.trialId} - {trial.phase}</div>
            <h3 className="font-heading text-lg font-black uppercase">{trial.title}</h3>
            <div className="flex gap-3 mt-2 font-mono text-xs text-black/60">
              <span><Hospital className="w-3 h-3 inline" /> {trial.sponsor}</span>
              <span><MapPin className="w-3 h-3 inline" /> {trial.location}</span>
              {trial.distanceText && (
                <span className={trial.withinRadius ? "text-lime-green" : "text-hot-coral"}>
                  <Navigation className="w-3 h-3 inline" /> {trial.distanceText}
                  {trial.durationText && ` (${trial.durationText})`}
                </span>
              )}
            </div>
          </div>
          <div className="text-right">
            {statusBadge}
            <div className="font-heading text-3xl font-black mt-2" style={{color: confidenceColor}}>
              {trial.confidence}%
            </div>
          </div>
        </div>
      ...
      {isExpanded && (
        <div className="border-t-4 border-black">
          <div className="bg-cyber-yellow px-4 py-2 border-b-2 border-black font-heading font-black uppercase text-sm">
            Eligibility Criteria
          </div>
          {trial.criteria.map(c => <CriterionRow key={c.id} criterion={c} />)}
          
          {trial.scoreComponents && (
            <ScoreBreakdown scores={trial.scoreComponents} confidence={trial.confidence} />
          )}
...
export default function ResultsPage() {
    ...
    async function calculateMatches() {
      ...
      try {
        ...
        const trialsWithDistances: TrialMatch[] = matchingTrials.map((trial, index) => {
          ...
          const withinRadius = distance <= trial.radiusKm;
          
          const criteria: CriterionExplanation[] = [
            { 
              id: "C1", 
              category: "Age", 
              criterion: `Age ${trial.minAge}-${trial.maxAge} years`, 
              status: patient.age >= trial.minAge && patient.age <= trial.maxAge ? "pass" : "fail", 
              patientValue: `${patient.age} years`, 
              ruleValue: `${trial.minAge}-${trial.maxAge} years`, 
              reasoning: patient.age >= trial.minAge && patient.age <= trial.maxAge ? "Within range" : "Outside range" 
            },
            { 
              id: "C2", 
              category: "Gender", 
              criterion: trial.gender === "All" ? "Any gender" : trial.gender, 
              status: trial.gender === "All" || patient.gender === trial.gender ? "pass" : "fail", 
              patientValue: patient.gender, 
              ruleValue: trial.gender, 
              reasoning: trial.gender === "All" || patient.gender === trial.gender ? "Matches" : "Does not match" 
            },
            { 
              id: "C3", 
              category: "Diagnosis", 
              criterion: trial.requiredDiagnoses.join(", "), 
              status: "pass", 
              patientValue: patient.diagnosis, 
              ruleValue: trial.requiredDiagnoses.join(", "), 
              reasoning: "Diagnosis matches", 
              isML: true 
            },
            { 
              id: "C4", 
              category: "ECOG", 
              criterion: `ECOG <= ${trial.ecogMax}`, 
              status: patient.ecogStatus <= trial.ecogMax ? "pass" : "fail", 
              patientValue: `ECOG ${patient.ecogStatus}`, 
              ruleValue: `<= ${trial.ecogMax}`, 
              reasoning: patient.ecogStatus <= trial.ecogMax ? "Within range" : "Exceeds limit" 
            },
            { 
              id: "C5", 
              category: "Geography", 
              criterion: `Within ${trial.radiusKm} km`, 
              status: withinRadius ? "pass" : "fail", 
              patientValue: `${distanceText}${durationText !== "Calculating..." ? ` (${durationText})` : ""}`, 
              ruleValue: `< ${trial.radiusKm} km`, 
              reasoning: withinRadius ? "Within travel distance" : "Too far from trial site" 
            },
          ];
          ...
          // Calculate composite scores using the new formula
          const ruleScore = calculateRuleScore(criteria);
          const embeddingScore = calculateEmbeddingScore(patient.diagnosis, trial.conditions);
          const llmScore = calculateLLMScore(criteria);
          const geoScore = calculateGeoScore(distance, trial.radiusKm);
          ...
          const confidence = calculateCompositeScore(scoreComponents);
  ...
  const sortedTrials = useMemo(() => 
    [...trials].sort((a, b) => b.confidence - a.confidence), 
    [trials]
  );
  
  const stats = useMemo(() => ({ 
    total: sortedTrials.length, 
    eligible: sortedTrials.filter(t => t.status === "pass").length, 
    avgConfidence: Math.round(sortedTrials.length ? sortedTrials.reduce((s, t) => s + t.confidence, 0) / sortedTrials.length : 0) 
  }), [sortedTrials]);
  
  const trialsForMap = useMemo(() => 
    trials.filter(t => t.coordinates).map(t => ({
      name: t.title, 
      location: t.location, 
      lat: t.coordinates!.lat, 
      lng: t.coordinates!.lng, 
      withinRadius: t.withinRadius || false, 
      distance: t.distanceText || t.distance + " km"
    })), 
    [trials]
  );
  ...
  return (
    <div className="min-h-screen bg-cream font-mono">
      <header className="bg-black text-white px-4 md:px-8 py-3 flex items-center justify-between border-brutal-b">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="px-3 py-2 bg-cyber-yellow text-black font-heading font-black text-sm border-2 border-black uppercase">Results</span>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-black text-white border-brutal shadow-brutal p-6">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="font-heading text-xl text-lime-green">{selectedPatient.id}</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="font-heading text-lg text-cyber-yellow">{selectedPatient.stage}</div>
                </div>
                <div>
            ...
            <div className="bg-lime-green border-brutal shadow-brutal p-6">
              <div className="flex items-center gap-2 mb-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  <span className="font-mono text-sm">Eligible</span>
                  <span className="font-heading text-2xl font-black bg-black text-lime-green px-2">{stats.eligible}</span>
                </div>
                <div className="flex justify-between">
            ...
            <div className="bg-[#111] text-white border-brutal shadow-brutal p-6">
              <div className="flex items-center gap-2 mb-4">
                <Stethoscope className="w-5 h-5 text-cyber-yellow" />
                <span className="font-heading font-black uppercase">Engine</span>
              </div>
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span className="font-mono text-xs">Rule Engine</span>
                  <span className="text-lime-green text-xs flex items-center gap-1">
                    <CheckCircle2 className="w-3 h-3" />ACTIVE
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="font-mono text-xs">ML Matcher</span>
                  <span className="text-lime-green text-xs flex items-center gap-1">
                    <CheckCircle2 className="w-3 h-3" />ACTIVE
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="font-mono text-xs">Geo Filter</span>
                  <span className="text-lime-green text-xs flex items-center gap-1">
                    <CheckCircle2 className="w-3 h-3" />ACTIVE
                  </span>
                </div>
            ...
            <div className="bg-white border-brutal shadow-brutal overflow-hidden">
              <div className="bg-cyber-yellow px-4 py-2 border-b-2 border-black">
                <span className="font-heading font-black uppercase text-sm">Map</span>
              </div>
              <div className="h-64">
                {isCalculating ? (
                  ...
                  <GoogleMaps 
                    patientLocation={`${selectedPatient.city}, ${selectedPatient.state}`} 
                    patientLat={patientCoords.lat} 
                    patientLng={patientCoords.lng} 
                    trials={trialsForMap} 
                    radiusMiles={RADIUS_KM} 
                  />
                ) : (
              ...
              <div className="p-3 bg-black/5 border-t-2">
                <span className="font-mono text-[10px]">Radius: {RADIUS_KM} km</span>
              </div>
            </div>
          </div>
          
          <div className="lg:col-span-3">
            <div className="bg-black text-white px-6 py-4 border-brutal border-b-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Target className="w-6 h-6 text-lime-green" />
                <h2 className="font-heading font-black text-xl uppercase">Ranked Matches</h2>
              </div>
              <span className="font-mono text-xs">
                <Calendar className="w-4 h-4 inline" /> {new Date().toLocaleDateString()}
              </span>
            </div>
            
            {isCalculating ? (
              <div className="p-12 flex flex-col items-center bg-white border-brutal">
                <Loader2 className="w-12 h-12 animate-spin mb-4" />
                <p className="font-mono text-sm">Calculating matches with composite scoring...</p>
              </div>
            ) : (
              <div className="space-y-4 mt-4">
                {sortedTrials.map(t => (
                  <TrialCard 
                    key={t.id} 
                    trial={t} 
                    isExpanded={expandedTrial === t.id} 
                    onToggle={() => setExpandedTrial(expandedTrial === t.id ? null : t.id)} 
                  />
                ))}
              </div>
            )}
            ...
            {stats.eligible > 0 && !isCalculating && (
              <div className="mt-6 bg-lime-green border-brutal shadow-brutal p-6">
                <div className="flex items-center gap-3 mb-4">
                  <CheckCircle2 className="w-6 h-6 text-black" />
                  <h3 className="font-heading font-black uppercase">Recommended</h3>
                </div>
                <p className="font-mono text-sm mb-4">
                  <strong>{sortedTrials[0].trialId}</strong> - {sortedTrials[0].confidence}% match
                </p>
